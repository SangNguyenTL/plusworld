...
vnb.Firebase = class {
  ...
  getComments(postId) {
    return this._getPaginatedFeed(`/comments/${postId}`,
        vnb.Firebase.COMMENTS_PAGE_SIZE, null, false);
  }
   ...
   */
  getPosts() {
    return this._getPaginatedFeed('/posts/', vnb.Firebase.POSTS_PAGE_SIZE);
  }
  ...
  getHomeFeedPosts() {
    return this._getPaginatedFeed(`/feed/${this.auth.currentUser.uid}`,
        vnb.Firebase.POSTS_PAGE_SIZE, null, true);
  }
  ...
  getUserFeedPosts(uid) {
    return this._getPaginatedFeed(`/people/${uid}/posts`,
        vnb.Firebase.USER_PAGE_POSTS_PAGE_SIZE, null, true);
  }
  ...
  loadUserPositions(uid) {
    return this.database.ref(`/people/${uid}/positions`).once('value') || [];
  }
  
  /**
   * Load a single user profile information
   */
  loadNameUserPositions(uid) {
    return this.database.ref(`/options/positions/${uid}`).once('value') || "None";
  }
