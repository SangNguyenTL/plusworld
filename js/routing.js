...
vnb.Router = class {
  ...
  constructor() {
    $(document).ready(() => {
      vnb.auth.waitForAuth.then(() => {
        ...
        // Configuring routes.
        const pipe = vnb.Router.pipe;
        const displayPage = this.displayPage.bind(this);
        const loadUser = userId => vnb.userPage.loadUser(userId);
        const showHomeFeed = () => vnb.feed.showHomeFeed();
        const showGeneralFeed = () => vnb.feed.showGeneralFeed();
        const clearFeed = () => vnb.feed.clear();
        const showPost = postId => vnb.post.loadPost(postId);

         ('/', pipe(showHomeFeed, null, true),
  ...
  displayPage(attributes, context) {
    ...
    vnb.Router.setLinkAsActive(context.canonicalPath);
    ...
    vnb.MaterialUtils.closeDrawer();
    vnb.Router.scrollToTop();
  }
